...
Middleware.prototype.parseMetadata = function(message) {
  var result = {
    ...
    user: this.slackClient.getUserName(message.message.user),
...
function addSuccessReaction(that, msgId, message) {
  return function(issueUrl) {
    var channel = message.item.channel,
        timestamp = message.item.ts,
        reaction = that.slackClient.successReaction,
        resolve, reject;
    ...
    log(msgId + ': adding ' + reaction);
    return that.slackClient.addSuccessReaction(channel, timestamp)
      .then(resolve, reject);
  };
}
