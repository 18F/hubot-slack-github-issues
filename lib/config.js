...
Config.prototype.checkForMisconfiguredRules = function(errors) {
  ...
  rulesByReaction = groupRulesBy(this.rules, 'reactionName');
  checkRulesByReaction(rulesByReaction, errors);
};
...
function checkRulesByReaction(rulesByReaction, errors) {
...
function checkForDuplicateRepositories(rules, reactionName, errors) {
  var duplicateRepos = rules.map(function(r) { return r.githubRepository; })
    .sort(function(lhs, rhs) { return lhs.localeCompare(rhs); })
...
function checkForDuplicateChannels(rules, reactionName, errors) {
  var duplicateChannels = rules.reduce(reduceChannels, [])
    .sort(sortStrings)
    .filter(detectDuplicates);
...
function reduceChannels(channels, rule) {
